...
- (id)initWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle viewController:(UIViewController *)vc isTransfer:(BOOL)flag ID:(int)projectID;
{
    ...
    if (self) {
        ...
        [self creatButtonsWithTitle:title cancelButtonTitle:cancelButtonTitle isTransfer:flag];
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle isTransfer:(BOOL)flag
{
    
    NSArray *shareButtonTitlesArray = [[NSArray alloc] init];
    NSArray *shareButtonImagesNameArray = [[NSArray alloc] init];

        shareButtonTitlesArray = @[@"微信朋友圈",@"微信好友",@"微博"];
        shareButtonImagesNameArray = @[@"share_friends",@"share_wechat",@"share_sina"];
    ...
    if (shareButtonImagesNameArray) {
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                [shareButton addTarget:self action:@selector(didClickOnImageIndex1:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)didClickOnImageIndex1:(UIButton *)button
{
    _share = [[shareDesc alloc] initWithFrame:CGRectMake(10, 100, 300, 210)];
    __weak ShareActivity * sa = self;
...
-(void)shareWithButton:(UIButton *)button
{
    if(self.vc != nil){
        ...
        if (button.tag==0)
        {
            [[UMSocialDataService defaultDataService]  postSNSWithTypes:@[UMShareToWechatTimeline] content:_share.contentTextView.text image:_share.projectImg.image location:nil urlResource:nil presentedController:_vc completion:^(UMSocialResponseEntity *response){
        ...
        }else if (button.tag==1)
        {
            ...
            [[[UIAlertView alloc] initWithTitle:@"转发" message:@"转发成功" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil] show];
        }
